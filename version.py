...
VERSION_NOTES = {
    ...
    "1.5.0": "MCP认证增强：支持HTTP Bearer Token双模式认证(Bearer推荐+URL参数兼容)、Token弹窗UI重构、状态标签优化、完整测试套件",
    "1.5.2": "配置治理与安全增强：SOAR配置全面转为后台数据库管理、首次配置引导、管理员密码修改与独立密码管理页面",
    "1.6.0": "关键结果提取与升级治理：剧本动作节点名称关键词、执行结果概览/关键结果分离、数据库迁移脚本、后台版本展示"
}
