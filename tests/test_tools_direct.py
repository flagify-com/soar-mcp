...
async def test_tools_directly():
    ...
    try:
        ...
        result = list_playbooks_detailed(limit=2)
        if result:
            data = json.loads(result)
            print(f"   📊 总计: {data.get('total', 0)} 个剧本")
        ...
        result = query_playbook_execution_params(playbook_id=playbook_id)
        if result:
            data = json.loads(result)
        ...
        result = execute_playbook_advanced(
        ...
        for i in range(3):
            print(f"   🔍 查询 #{i+1}:")
            result = query_playbook_execution_status(activity_id=activity_id)
            if result:
                data = json.loads(result)
                print(f"      - 状态: {data.get('status')}")
                print(f"      - 消息: {data.get('message')}")
        ...
        result = query_playbook_execution_result(activity_id=activity_id)
        if result:
            data = json.loads(result)
